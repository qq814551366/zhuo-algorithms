...
## 两个过程
 - 递的过程，把大问题拆解成子问题
 - 归的过程，把子问题回归到大问题
 - 具体应该在递归前（递）操作，还是递归后（归）操作，则要看具体问题了
...
递归函数的定义注意事项：
1. 函数定义针对的一定是元素，例如 二叉树的前中后序遍历，递归函数操作的是某一节点数据、
2. 并且传递的参数必包含下一个数据的指针（例如参数中必带**索引i**+1，**链表节点**->next，）

确定递归终止条件、确定边界条件的思考范围:
1. 结束时的跳出条件
2. 多递归时（例如矩阵上下左右移动），是否存在走过的路径重复走，如何做标记
3. 参数的取值范围是否会超出，超出时应该怎么做
